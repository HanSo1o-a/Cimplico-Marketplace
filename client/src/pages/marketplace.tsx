...
const Marketplace = () => {
  ...
  // 分类过滤（只在前端过滤）
  let filteredData = category && category !== "all"
  ...
  if (searchQuery && searchQuery.trim() !== "") {
    ...
    filteredData = filteredData.filter((item: any) => {
      ...
      if (item.tags && Array.isArray(item.tags)) {
        return item.tags.some((tag: string) => tag.toLowerCase().includes(query));
      }
      return false;
    });
    console.log("[Marketplace] 搜索查询:", searchQuery, "过滤后商品:", filteredData);
  }
